...
const MemberSideBar = ({ member, navLinks }: Props) => {
  ...
  const translation = useTranslations('Member');
  return (
      ...
      <CardBody className="w-full flex-1">
        <div className="flex flex-col items-center">
          <div className="flex items-center gap-1">
            <div className="sm:text-sm md:text-xl lg:text-2xl">
        ...
        <nav className="ml-2 flex flex-col gap-2 p-2 text-sm md:text-xl lg:text-2xl">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
