    ...
    <div className="cursor-pointer" onClick={onOpen}>
      ...
      {!photo?.isApproved && role !== 'ADMIN' && (
        <div className="absolute bottom-2 w-full bg-slate-200 p-1">
          <div className="flex justify-center font-semibold text-danger">Awaiting approval</div>
        </div>
      )}
      {role === 'ADMIN' && (
        <div className="mt-2 flex flex-row gap-2">
          <Button
            onPress={() => approvePhotoHandler(photo.id)}
            color="success"
            variant="bordered"
            fullWidth
          >
          ...
          <Button
            onPress={() => rejectPhotoHandler(photo)}
            color="danger"
            variant="bordered"
            fullWidth
          >
        ...
        body={
          <>
            {photo?.publicId ? (
              ...
              <Image width={750} src={photo?.url || '/images/user.png'} alt="Image of user" />
            )}
          </>
        }
      />
