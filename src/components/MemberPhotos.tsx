...
const MemberPhotos = ({ photos, editing, mainImageUrl }: Props) => {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 p-5 md:grid-cols-3 lg:grid-cols-5">
      {photos &&
        photos.map((photo) => (
          <div key={photo.id} className="relative">
            ...
            {editing && (
              <>
                <div onClick={() => onSetMain(photo)} className="absolute left-3 top-3 z-50">
                  <StarButton
                ...
                <div onClick={() => onDelete(photo)} className="absolute right-3 top-3 z-50">
                  <DeleteButton
                    loading={
