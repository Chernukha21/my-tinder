...
export const useFilters = () => {
    ...
    const {pageNumber, pageSize, setPage, totalCount} = usePaginationStore(useShallow(
        state => ({
    ...
    const handleByGenderSelect = (value: string) => {
        if (gender.includes(value)) setFilters('gender', gender.filter(gender => gender !== value));
